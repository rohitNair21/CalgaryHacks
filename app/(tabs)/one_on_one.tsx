...
const OneOnOne = () => {
    ...
    const [messages, setMessages] = useState<{ sent_text: string, sender: 'user' | 'bot', timestamp: string }[]>([]);
    ...
    const sendMessage = async () => {
        if (message.trim()) {
            ...
            setMessage('');
            if (!agentName) {
                setAgentName('Agent Smith'); // Placeholder name
            }
        }
    };
    ...
    return (
        <SafeAreaView style={[styles.safeContainer, { backgroundColor: theme.background }]}> 
            <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={[styles.container, { backgroundColor: theme.background }]}> 
                {agentName && <Text style={[styles.agentName, { color: theme.text }]}>Connected with {agentName}</Text>}
